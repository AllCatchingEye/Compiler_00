...
-- Konstruktoren.
data AExpr
  = AExprInt Integer -- ganzzahlige Konstante
...
-- Konstruktoren.
data BExpr
  = BExprBool Bool -- Boolesche Konstante
...
declareVar p@(ProgramState memory _ _) v
...
updateVar p@(ProgramState memory _ _) var val
...
varValue (ProgramState memory _ _) var = Map.lookup var memory
